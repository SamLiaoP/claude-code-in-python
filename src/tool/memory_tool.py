...
#
# 用途：AI 透過此 tool 讀寫使用者的跨 Session 記憶
...
class MemoryReadTool(Tool):
    name = "memory_read"
    description = "讀取使用者的跨 Session 記憶。不給 key 則列出全部記憶。"
    parameters = {
    ...
    async def execute(self, params: dict[str, Any], ctx: ToolContext) -> ToolResult:
        ...
        key = params.get("key")
        memories = await memory_read(ctx.user_id, key)
        if not memories:
...
class MemoryWriteTool(Tool):
    name = "memory_write"
    description = "寫入使用者的跨 Session 記憶。用於記住使用者偏好、研究脈絡等。"
    parameters = {
    ...
    async def execute(self, params: dict[str, Any], ctx: ToolContext) -> ToolResult:
        ...
        await memory_write(ctx.user_id, key, value)
