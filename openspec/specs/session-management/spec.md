...
系統 MUST 提供每人一個跨 Session 的記憶空間，AI 可透過 `memory_read` / `memory_write` tool 操作。存儲於 SQLite `user_memories` 表。

#### Scenario: AI 寫入使用者記憶
- **WHEN** AI 呼叫 `memory_write(key="research_focus", value="心血管藥物")`
- **THEN** 該記憶存入 user_memories 表，綁定當前 user_id

#### Scenario: 新 Session 讀取先前記憶
- **WHEN** 使用者在新 Session 中，AI 呼叫 `memory_read(key="research_focus")`
- **THEN** 系統回傳先前存入的 `"心血管藥物"`
